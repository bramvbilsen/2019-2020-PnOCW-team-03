...
export default class Line {
    ...
    lineAbove(line: Line,index: number) { 
        ...
        let rico = (this.endPoints[1].y-this.endPoints[0].y)/(this.endPoints[1].x-this.endPoints[0].x)
        if (this.endPoints[index].y+rico*(otherPoint.x - this.endPoints[index].x) < otherPoint.y) {
          return true;
    ...
    surroundingCircle(line2: Line,line3: Line) {
      ...
      var radius = Math.sqrt(
          Math.pow(this.a.x - middPoint.x, 2) + Math.pow(this.a.y - middPoint.y, 2));
    ...
    angle(line2: Line,index: number) { 
      ...
      else {points.push(points2[1]);}
      
      let theta = (Math.atan2(points[1].y - over.y, points[1].x - over.x)-Math.atan2(points[0].y - over.y, points[0].x - over.x)) * 180 / Math.PI;
